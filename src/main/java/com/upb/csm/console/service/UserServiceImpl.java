...
import com.upb.csm.console.repository.UserRepository;
...
public class UserServiceImpl implements UserService{
	...
	private boolean checkUsernameAvailable(CSMUser user) throws Exception {
		Optional<CSMUser> userFound = repository.findByUsername(user.getUsername());
	...
	private boolean checkPasswordValid(CSMUser user) throws Exception {
	...
	public CSMUser getUserById(Long id) throws UsernameOrIdNotFound {
		return repository.findById(id).orElseThrow(() -> new UsernameOrIdNotFound("El Id del usuario no existe."));
	}

	public CSMUser updateUser(CSMUser fromUser) throws Exception {
		CSMUser toUser = getUserById(fromUser.getId());
	 ...
	 * @param to
	 */
	protected void mapUser(CSMUser from,CSMUser to) {
	...
	public CSMUser changePassword(ChangePasswordForm form) throws Exception {
		CSMUser user = getUserById(form.getId());
	...
	private CSMUser getLoggedUser() throws Exception {
		//Obtener el usuario logeado
		Object principal = SecurityContextHolder.getContext().getAuthentication().getPrincipal();
